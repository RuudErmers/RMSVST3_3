...
uses Forms,UVST3Processor,Vst3Base,UVSTBase,UCDataLayer,Generics.Collections,ExtCtrls,Types,SysUtils,UVST3Instrument,UVST3InstrumentBase;
...
type TVST3Program = class
 ...
 IVST3Controller = interface(IVSTBase)
   ...
   TVST3ControllerBase = class(TVSTBase,IVST3Controller,ICBController)
   private
        FCurProgram:integer;
        ...
        procedure ConnectToProcessor;
        procedure SetProgramsAndParameters;
   protected
        ...
        procedure OnProgramChange(prgm:integer);
        function  GetEditorClass: TFormClass;virtual;
        procedure OnEditClose;
        procedure OnEditIdle;
        procedure UpdateControllerParameter(id:integer;value:double);
        ...
        procedure MidiEvent(byte0, byte1, byte2: integer);

        procedure doProgramChange(prgm:integer);
        procedure OnSize(newSize:TRect);virtual;
        procedure SendMessageToProcessor(msg:TBytes);
        procedure Connected(other: IConnectionPoint);override;

   public
        procedure SetVST3Instrument(instr:TVST3Instrument);
...
procedure TVST3ControllerBase.SetVST3Instrument(instr: TVST3Instrument);
begin
  WriteLog('Controller knows FVST3Instrument...');

  FVST3InstrumentBase:=TVST3InstrumentBase(instr);
  instr.SetCBController(self);
  // execute previous 'failed' methods
  SetProgramsAndParameters
//  OnProgramChange(0);
end;
...
begin
  WriteLog('TVST3ControllerBase.Initialize');
// even een testje...  SetProgramsAndParameters;
end;

procedure TVST3ControllerBase.SetProgramsAndParameters;
VAR title:string;
    i:integer;
begin
  ...
  SetParameters(FParameters);                  // << since we are connected to the Processor, we could clone FParameters from there
                                               // << also, since FVST3InstrumentBase is now valid we could simplify the code from SetParameters
// removed  OnInitialize;
...
procedure TVST3ControllerBase.SetParameters(params:TVST3ParameterArray);
begin
...
begin
  FeditorForm:=form;
  FVST3Instrument.OpenEditor(EditorForm);
  SetIdleTimer(true);
  ResendParameters;
end;

function TVST3ControllerBase.FVST3Instrument: TVST3Instrument;
begin
...
begin
  case msg[0] of
    MSG_MIDIINT: MidiEvent(msg[1],msg[2],msg[3]);
  end;
...
begin
  ...
  begin
    ...
    UpdateControllerParameter(id,Fparameters.params[i].value);
...
begin
  ...
  for i:=0 to count-1 do with Fparameters.params[i] do
    if dirty then
    begin
      UpdateControllerParameter(id,value);
      dirty:=false
    end;
...
procedure TVST3ControllerBase.UpdateControllerParameter(id: integer;  value: double);
begin
  FVST3Instrument.UpdateControllerParameter(id,value);
end;
...
begin
  FVST3Instrument.OnEditClose
end;

procedure TVST3ControllerBase.OnEditIdle;
begin
  FVST3Instrument.OnEditIdle
end;
...
procedure TVST3ControllerBase.MidiEvent(byte0, byte1, byte2: integer);
begin
  FVst3Instrument.OnMidiEvent(false,byte0, byte1, byte2);
end;

procedure TVST3ControllerBase.OnProgramChange(prgm: integer);
begin
  if FVst3Instrument<>NIL then
    FVst3Instrument.OnProgramChange(prgm);
end;
...
procedure TVST3ControllerBase.Connected(other: IConnectionPoint);
begin
  ConnectToProcessor;
end;

procedure TVST3ControllerBase.ConnectToProcessor;
VAR bytes:TBytes;
    i,l:integer;
begin
  SetLength(bytes,9);
  bytes[0]:=MSG_CONTROLLER_ATTACH;
  l:=longint(self);
  for i:=0 to 7 do
  begin
    bytes[i+1]:=l and $FF;
  ...
  SendMessageToProcessor(bytes);
end;

procedure TVST3ControllerBase.ControllerParameterSetValue(id: integer; value: double);
