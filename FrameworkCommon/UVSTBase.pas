...
uses Forms, Generics.Collections,UCDataLayer,Vst3Base,SysUtils,UVST3InstrumentBase;
...
type
     ...
     TVST3InstrumentClass = class of TVST3InstrumentBase;
     TVSTPluginDef =  record
                              vst3processorid:TGUID;
                              vst3controllerid:TGUID;
                              vst3instrumentclass: TVST3InstrumentClass;
    ...
    IVSTBase = interface
        ...
        procedure OnCreate(pluginInfo:TVSTInstrumentInfo;isProcessor:boolean);
        procedure Initialize(context: FUnknown);
        ...
        procedure Connected(other: IConnectionPoint);
     end;
     ...
     private
        ...
        procedure Initialize(context: FUnknown);overload;
        procedure SetMessageSendHandler(handler:TSendMessageHandler);
     protected
        FVST3InstrumentBase:TVST3InstrumentBase;
        procedure Initialize;overload;virtual;
        procedure ReceiveMessage(msg:TBytes);virtual;
        procedure Connected(other: IConnectionPoint);virtual;
     public
        function Host: IHostApplication;
        procedure SendMessage(msg:TBytes);
        function GetPluginInfo:TVSTInstrumentInfo;

        procedure OnCreate(pluginInfo:TVSTInstrumentInfo;isProcessor:boolean); virtual;
        constructor Create; virtual;
     end;

const MSG_MIDIOUT = 0;
const MSG_MIDIINT = 1;
const MSG_CONTROLLER_ATTACH = 2;
...
procedure TVSTBase.Connected(other: IConnectionPoint);
begin
//;
end;
...
procedure TVSTBase.Initialize(context: FUnknown);
begin
...
procedure TVSTBase.OnCreate(pluginInfo: TVSTInstrumentInfo;isProcessor:boolean);
begin
  FPluginInfo:=pluginInfo;
  if isProcessor then
    FVST3InstrumentBase:=pluginInfo.PluginDef.vst3instrumentclass.create;
end;
...
function TVST3ParameterArray.ParmLookup(id: integer): integer;
VAR i:integer;
begin
  for i:=0 to length(params)-1 do
    if params[i].id = id then begin result:=i; exit; end;
  result:=-1;
end;
